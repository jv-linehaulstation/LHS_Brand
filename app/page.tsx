...
// Reusable section kickers (one orange mono label + rule).
function Kick({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-3 font-mono text-[12px] uppercase tracking-[0.28em] text-fuel">
      <span className="h-px w-7 bg-fuel" aria-hidden /> {children}
    </div>
  );
}
...
export default function Home() {
  return (
    <main className="min-h-screen bg-ink">
      ...
      <section className="relative flex min-h-[100dvh] items-center overflow-hidden px-5 pb-20 pt-32 sm:px-8">
        <BackgroundVideo
          src={HERO_VIDEO}
        ...
        <div className="relative mx-auto w-full max-w-site">
          <Reveal>
            <Kick>Re-Think Trucking</Kick>
          </Reveal>
          <Reveal as="h1" delay={90} className="mt-5 max-w-[15ch] font-display text-[clamp(42px,8.5vw,128px)] font-black uppercase leading-[0.86] tracking-[-0.03em] text-white">
            Home Base For The People Who Move{" "}
            <span className="outline-head" style={{ ["--ac" as string]: "#F07820" }}>America.</span>
          </Reveal>
          <Reveal delay={180}>
            <p className="mt-6 max-w-[48ch] text-pretty font-body text-[clamp(17px,1.8vw,22px)] leading-relaxed text-[#dadada]">
              A national network of member-only, shared-use Private Terminals — real terminals, real
              amenities, a relay that lowers the cost of every mile.
            </p>
          </Reveal>
      ...
      <section className="px-5 py-[clamp(70px,11vh,140px)] sm:px-8" style={{ background: WHITE }}>
        <div className="mx-auto max-w-site">
          ...
          <div className="mt-8 grid gap-[clamp(28px,5vw,64px)] lg:grid-cols-[1.3fr_0.9fr] lg:items-start">
            <div>
              <Reveal as="h2" className="font-display text-[clamp(34px,5.6vw,84px)] font-black uppercase leading-[0.9] tracking-[-0.025em]">
      ...
      <section className="bg-ink px-5 py-[clamp(70px,11vh,140px)] sm:px-8">
        <div className="mx-auto max-w-site">
          ...
          <Reveal as="h2" className="mt-3 font-display text-[clamp(34px,5.6vw,84px)] font-black uppercase leading-[0.9] tracking-[-0.025em] text-white">
      ...
      {/* ============ 4. THE GALLERY [W] — full-bleed full-screen slider ============ */}
      <section className="py-[clamp(70px,11vh,140px)]" style={{ background: WHITE }}>
        <div className="mx-auto mb-10 max-w-site px-5 sm:px-8">
          <Reveal><Kick>The Gallery</Kick></Reveal>
          <Reveal as="h2" className="mt-3 font-display text-[clamp(34px,5.6vw,84px)] font-black uppercase leading-[0.9] tracking-[-0.025em]">
      ...
      {/* ============ 5. FIND YOUR LANE [B] — desc left / image right ============ */}
      ...
      {/* ============ 9. CLOSING CTA [B] — full-bleed parallax ============ */}
      <section id="contact" className="relative flex min-h-[80vh] items-center overflow-hidden px-5 py-[clamp(80px,12vw,150px)] sm:px-8">
        ...
        <div className="relative mx-auto w-full max-w-site">
          ...
          <Reveal as="h2" className="mt-4 max-w-[18ch] font-display text-[clamp(34px,5.6vw,84px)] font-black uppercase leading-[0.9] tracking-[-0.025em] text-white">
            Build The Future Of Freight With Us.
          </Reveal>
          <Reveal delay={120} className="mt-8 flex flex-wrap gap-3.5">
