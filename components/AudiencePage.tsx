          ...
          <div className="mt-9 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {a.amenities.tiles.map((t, i) => (
              <Reveal
                key={i}
                delay={(i % 4) * 70}
                className="group rounded-card border border-chrome/15 bg-panel p-5 transition-colors hover:bg-[#181818]"
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[14px] transition-transform group-hover:translate-x-0.5" style={{ color: a.accent }}>
                    +
                  </span>
                </div>
                <p className="mt-2 font-body text-[14px] leading-snug text-chrome">{t.blurb}</p>
              </Reveal>
            ))}
          </div>
          <Reveal delay={120}>
