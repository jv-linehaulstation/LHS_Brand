    ...
    <div className="grid gap-5 sm:grid-cols-3">
      {items.map((it, i) => (
        <Reveal key={it.title} delay={i * 80}>
          <Spotlight className="group h-full overflow-hidden rounded-[18px] border" glow={`${accent}2b`} style={{ borderColor: "#1C1C20", background: "#16161A" }}>
            <div className="relative aspect-[4/3] overflow-hidden">
              ...
              <div className="absolute left-4 top-4 inline-flex items-center rounded-full px-3 py-1 font-label text-[12px] uppercase tracking-[0.14em] text-ink transition-transform duration-300 ease-out group-hover:-translate-y-0.5" style={{ background: accent }}>
                {badge(i)}
              </div>
            </div>
            <figcaption className="relative z-[3] p-5">
