...
export function PrimaryCTA({
  ...
  return (
    <a
      href={href}
      className={`${label} group relative overflow-hidden rounded-btn px-[30px] py-[17px] text-ink shadow-[0_14px_30px_rgba(0,0,0,0.45)] transition duration-300 hover:-translate-y-0.5 hover:brightness-[1.08] active:scale-[0.97]`}
      style={{ background: `linear-gradient(135deg, ${accent}, ${accentDark})` }}
    >
      {/* one-pass sheen on hover — GPU transform sweep (not `left`) */}
      <span
        aria-hidden
        className="pointer-events-none absolute inset-y-0 left-0 w-1/3 -translate-x-[160%] -skew-x-12 bg-white/25 opacity-0 transition-[transform,opacity] duration-500 ease-out group-hover:translate-x-[420%] group-hover:opacity-100"
      />
...
export function GhostCTA({
  ...
  return (
    <a
      href={href}
      className={`${label} group rounded-btn border border-chrome/30 bg-carbon/70 px-[30px] py-[17px] text-white transition duration-300 hover:border-[var(--ac)] hover:bg-carbon active:scale-[0.97]`}
