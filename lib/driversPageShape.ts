// Client-safe shape + transform for the DriversPage global. Deliberately has NO
// @payload-config / getPayload import so it can run in the browser (Live Preview
// merges the RAW Payload doc client-side, then resolveDriversDoc turns it into
// the render-ready DriversPageContent). The server fetch lives in lib/driversPage.ts.
//
// Copy source of truth: lib/onehome.ts (ONEHOME) + lib/audiences.ts
// (audiences.drivers) + the section consts from components/OneHomePage.tsx.
...
export const SEED: DriversPageContent = {
  // 1 Hero
  heroVideoUrl:
...
// Map a CMS array via `f`; empty CMS array → seed.
